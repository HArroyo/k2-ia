...
from .configuration_secvisor import SecVisorConfig 
from .configuration_secvisor import SecVisorLanguageConfig
from .configuration_secvisor import SecVisorVisionConfig
...
_CONFIG_FOR_DOC = "SecVisorConfig"
...
class SecVisorLearnedPositionalEmbedding(nn.Embedding):
    """
    This module learns positional embeddings up to a fixed maximum size.
    """

    def __init__(self, num_embeddings: int, embedding_dim: int):
        # SecVisor is set up so that if padding_idx is specified then offset the embedding ids by 2
...
class SecVisorScaledWordEmbedding(nn.Embedding):
    """
...
class SecVisorAttention(nn.Module):
    ...
    def __init__(
        ...
        config: Optional[SecVisorLanguageConfig] = None,
    ):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
...
class SecVisorFlashAttention2(SecVisorAttention):
    """
    SecVisor flash attention module. This module inherits from `SecVisorAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # SecVisorFlashAttention2 attention does not support output_attentions
        if output_attentions:
            raise ValueError("SecVisorFlashAttention2 attention does not support output_attentions")
...
class SecVisorSdpaAttention(SecVisorAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "SecVisorModel is using SecVisorSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
SECVISOR_ATTENTION_CLASSES = {
    "eager": SecVisorAttention,
    "sdpa": SecVisorSdpaAttention,
    "flash_attention_2": SecVisorFlashAttention2,
}


class SecVisorEncoderLayer(nn.Module):
    def __init__(self, config: SecVisorLanguageConfig):
        super().__init__()
        self.embed_dim = config.d_model

        self.self_attn = SECVISOR_ATTENTION_CLASSES[config._attn_implementation](
...
class SecVisorDecoderLayer(nn.Module):
    def __init__(self, config: SecVisorLanguageConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = SECVISOR_ATTENTION_CLASSES[config._attn_implementation](
            self.embed_dim,
...
class SecVisorLanguagePreTrainedModel(PreTrainedModel):
    config_class = SecVisorLanguageConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _keys_to_ignore_on_load_unexpected = ["encoder.version", "decoder.version"]
    _no_split_modules = [r"SecVisorEncoderLayer", r"SecVisorDecoderLayer"]
...
class SecVisorEncoder(SecVisorLanguagePreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`SecVisorEncoderLayer`].

    Args:
        config: SecVisorLanguageConfig
        embed_tokens (nn.Embedding): output embedding
    """

    def __init__(self, config: SecVisorLanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_tokens = SecVisorScaledWordEmbedding(
        ...
        self.embed_positions = SecVisorLearnedPositionalEmbedding(
            config.max_position_embeddings,
            embed_dim,
        )
        self.layers = nn.ModuleList([SecVisorEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class SecVisorDecoder(SecVisorLanguagePreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`SecVisorDecoderLayer`]
    ...
    def __init__(self, config: SecVisorLanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([SecVisorDecoderLayer(config) for _ in range(config.decoder_layers)])
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self._use_sdpa = config._attn_implementation == "sdpa"
...
class SecVisorLanguageModel(SecVisorLanguagePreTrainedModel):
    ...
    def __init__(self, config: SecVisorLanguageConfig):
        ...
        self.encoder = SecVisorEncoder(config, self.shared)
        self.decoder = SecVisorDecoder(config, self.shared)
    ...
    def forward(
        ...
        # different to other models, SecVisor automatically creates decoder_input_ids from
...
class SecVisorLanguageForConditionalGeneration(SecVisorLanguagePreTrainedModel, GenerationMixin):
    base_model_prefix = "model"
    ...
    def __init__(self, config: SecVisorLanguageConfig):
        super().__init__(config)
        self.model = SecVisorLanguageModel(config)
...
@dataclass
class SecVisorSeq2SeqLMOutput(ModelOutput):
    """
    Base class for SecVisor-v6 model's outputs that also contains : pre-computed hidden states that can speed up sequential
    decoding.
...
SECVISOR_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`SecVisorConfig`] or [`SecVisorVisionConfig`]):
...
@add_start_docstrings(
    "The bare SecVisor-v6 Model outputting raw hidden-states without any specific head on top.",
    SECVISOR_START_DOCSTRING,
)
class SecVisorPreTrainedModel(PreTrainedModel):
    config_class = SecVisorConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _skip_keys_device_placement = "past_key_values"
...
SECVISOR_INPUTS_DOCSTRING = r"""
    Args:
        ...
        pixel_values (`torch.FloatTensor` of shape `(batch_size, num_channels, image_size, image_size)):
            ...
            [`AutoImageProcessor`]. See [`CLIPImageProcessor.__call__`] for details ([]`SecVisorProcessor`] uses
...
@add_start_docstrings(
    """The SECVISOR vision model without any head""",
    SECVISOR_START_DOCSTRING,
)
class SecVisorVisionModel(SecVisorPreTrainedModel):
    def __init__(self, config: SecVisorVisionConfig):
        super().__init__(config)
...
@add_start_docstrings(
    """The SECVISOR vision model with projection layer""",
    SECVISOR_START_DOCSTRING,
)
class SecVisorVisionModelWithProjection(SecVisorPreTrainedModel):
    def __init__(self, config: SecVisorVisionConfig):
        super().__init__(config)
        assert config.model_type == 'davit', 'only DaViT is supported for now'
        self.vision_tower = DaViT.from_config(config=config)
...
@add_start_docstrings(
    """The SECVISOR model which consists of a vision backbone and a language model.""",
    SECVISOR_START_DOCSTRING,
)
class SecVisorForConditionalGeneration(SecVisorPreTrainedModel):
    ...
    def __init__(self, config: SecVisorConfig):
        super().__init__(config)
        ...
        language_model = SecVisorLanguageForConditionalGeneration(config=config.text_config)
    ...
    @add_start_docstrings_to_model_forward(SECVISOR_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=SecVisorSeq2SeqLMOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, SecVisorSeq2SeqLMOutput]:
        r"""
        Args:
            labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
        ...
        >>> import requests
        >>> from transformers import AutoProcessor, SecVisorForConditionalGeneration

        >>> model = SecVisorForConditionalGeneration.from_pretrained("microsoft/SecVisor-v6-large")
        >>> processor = AutoProcessor.from_pretrained("microsoft/SecVisor-v6-large")
        ...
        return SecVisorSeq2SeqLMOutput(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            decoder_hidden_states=outputs.decoder_hidden_states,
