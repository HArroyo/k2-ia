...
"""
Processor class for SecVisor-v6.
"""
...
class SecVisorProcessor(ProcessorMixin):
    r"""
    Constructs a SecVisor processor which wraps a SecVisor image processor and a SecVisor tokenizer into a single processor.

    [`SecVisorProcessor`] offers all the functionalities of [`CLIPImageProcessor`] and [`BartTokenizerFast`]. See the
    [`~SecVisorProcessor.__call__`] and [`~SecVisorProcessor.decode`] for more information.
    ...
    def __init__(
        ...
        self.post_processor = SecVisorPostProcesser(tokenizer=tokenizer)
    ...
    def __call__(
        ...
        if images is None:
            raise ValueError("`images` are expected as arguments to a `SecVisorProcessor` instance.")
        if text is None:
            logger.warning_once(
                "You are using SecVisor-v6 without a text prompt."
            )
            text = ""
    ...
    # Copied from transformers.models.clip.processing_clip.CLIPProcessor.batch_decode with CLIP->SecVisor
    ...
    # Copied from transformers.models.clip.processing_clip.CLIPProcessor.decode with CLIP->SecVisor
    ...
    @property
    # Copied from transformers.models.clip.processing_clip.CLIPProcessor.model_input_names with CLIP->SecVisor
...
class SecVisorPostProcesser(object):
    r"""
    SecVisor-v6 post process for converting text prediction to various tasks results. 
